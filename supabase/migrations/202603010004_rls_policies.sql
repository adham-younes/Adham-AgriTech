...
drop policy if exists "org owner update org" on public.organizations;
drop policy if exists "org owner insert org" on public.organizations;
...
drop policy if exists "members read farms" on public.farms;
drop policy if exists "members write farms" on public.farms;
create policy "members read farms" on public.farms for select using (public.is_org_member(org_id));
create policy "members write farms" on public.farms for all using (public.is_org_member(org_id)) with check (public.is_org_member(org_id));

drop policy if exists "members read fields" on public.fields;
drop policy if exists "members write fields" on public.fields;
...
drop policy if exists "members read weather" on public.weather_snapshots_daily;
...
create policy "service manages weather" on public.weather_snapshots_daily for all to service_role using (true) with check (true);

drop policy if exists "members read irrigation" on public.irrigation_recommendations_daily;
...
create policy "service manages irrigation" on public.irrigation_recommendations_daily for all to service_role using (true) with check (true);

drop policy if exists "members read alerts" on public.alerts;
drop policy if exists "service manages alerts" on public.alerts;
create policy "members read alerts" on public.alerts for select using (
  exists (select 1 from public.fields fd join public.farms f on f.id = fd.farm_id where fd.id = field_id and public.is_org_member(f.org_id))
);
create policy "service manages alerts" on public.alerts for all to service_role using (true) with check (true);

drop policy if exists "members read ndvi" on public.satellite_ndvi_timeseries;
...
create policy "service manages ndvi" on public.satellite_ndvi_timeseries for all to service_role using (true) with check (true);
...
drop policy if exists "service manages reports" on public.reports;
...
drop policy if exists "service writes usage" on public.usage_events;
